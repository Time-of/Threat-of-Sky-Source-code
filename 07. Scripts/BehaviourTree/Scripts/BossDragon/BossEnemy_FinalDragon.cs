 ...
 * 드래곤 보스입니다.
 */
public class BossEnemy_FinalDragon : ABossBase
{
	[Header("드래곤 공격 패턴")]
	...
	[SerializeField]
	private AudioClip Clip_TailAtk;
	...
	[SerializeField]
	private ParticleSystem VFX_Bite;

	[SerializeField]
	private ParticleSystem VFX_TailAtk;
	...
	public override void TryAttack(int AttackType)
	{
		...
		switch (AttackType)
		{
			...
			case 1:
				...
				break;
			case 2:
				photonView.RPC("RpcSetAnimTrigger", RpcTarget.AllViaServer, "SK_TailAtk");
				break;
			default:
				break;
		}
	...
	{
		CharacterGameplay.CharacterGameplayHelper.PlayVfx(VFX_Bite, BreathPoint.position, Quaternion.identity, null);
	...
	public void SpawnBreathZone()
	{
		...
		DamageZone SpawnedZone = DamageHelper.SpawnDamageZone(DZ_Breath,
			BreathPoint.position + BreathPoint.transform.forward * 10.0f,
	...
	{
		CharacterGameplay.CharacterGameplayHelper.PlayVfx(VFX_TailAtk, transform.position, Quaternion.identity, null);

		SoundManager.Instance.SpawnSoundAtLocation(Clip_TailAtk, transform.position, ESoundGroup.SFX, 0.8f, 0.05f, AudioRolloffMode.Linear);

		DamageHelper.SpawnAutoStartDamageZone(DZ_TailAtk, transform.position + transform.forward * 4.0f, transform.rotation, AtkPower, 1.0f, this.gameObject);
		DamageHelper.ApplyRadialDamage(transform.position, BiteAtkRadius, AtkPower * 1.3f, "Player", true, this.gameObject);

		foreach (Collider Col in Physics.OverlapSphere(transform.position, BiteAtkRadius, 1 << LayerMask.NameToLayer("Player")))
		{
			if (Col.gameObject.TryGetComponent<ACharacterBase>(out ACharacterBase PlayerCharacter))
			{
				PlayerCharacter.GetMovementComponent().AddKnockback(Vector3.up * 3.0f + transform.forward * 8.0f);
			}
		}
	}
